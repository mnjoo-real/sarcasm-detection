#!/usr/bin/env python
"""Tests the impact of adding MFCC and/or mel-spectrogram summary features to
the current best feature set, for either corpus.
"""
import argparse
...
MELSPEC_COLS = [f"mel{i}_{stat}" for i in range(1, 41) for stat in ("mean", "std")]
...
def loso_mustard(df: pd.DataFrame, cols: list, label: str) -> None:
    ...
    print(f"{label:<36} n={len(cols):>3}  BBT={f1s['BBT']:.3f}  FRIENDS={f1s['FRIENDS']:.3f}  balanced={balanced:.3f}")
...
def groupkfold_podsarc(df: pd.DataFrame, cols: list, label: str, n_splits: int = 5) -> None:
    ...
    print(f"{label:<36} n={len(cols):>3}  F1={np.mean(f1s):.3f} (+/- {np.std(f1s):.3f})")
...
def main() -> None:
    ...
    if args.corpus == "mustard":
        ...
        melspec = pd.read_csv("data/melspec_features.csv")
        df = features.merge(mfcc, on="id", how="inner").merge(melspec, on="id", how="inner")
        loso_mustard(df, MUSTARD_BEST_23, "best 23")
        loso_mustard(df, MUSTARD_BEST_23 + MFCC_COLS, "best 23 + MFCC (51)")
        loso_mustard(df, MUSTARD_BEST_23 + MELSPEC_COLS, "best 23 + mel-spectrogram (103)")
        loso_mustard(df, MUSTARD_BEST_23 + MFCC_COLS + MELSPEC_COLS, "best 23 + MFCC + mel-spec (131)")
    else:
        ...
        melspec = pd.read_csv("data/podsarc_melspec_features.csv").rename(columns={"id": "nid"})
        df = harmonic.merge(labels, on="nid").merge(mfcc, on="nid").merge(melspec, on="nid")
        groupkfold_podsarc(df, PODSARC_VALIDATED_15, "PodSarc-validated 15")
        groupkfold_podsarc(df, PODSARC_VALIDATED_15 + MFCC_COLS, "+ MFCC (43)")
        groupkfold_podsarc(df, PODSARC_VALIDATED_15 + MELSPEC_COLS, "+ mel-spectrogram (95)")
        groupkfold_podsarc(df, PODSARC_VALIDATED_15 + MFCC_COLS + MELSPEC_COLS, "+ MFCC + mel-spec (123)")
